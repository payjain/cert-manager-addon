...
```bash
for cluster in $(oc get managedcluster -o name | sed 's|managedcluster/||'); do
  echo "=== $cluster ==="
  EXISTING_NS="openshift-operators"  # adjust if your Policy used a different namespace
  oc --context=$cluster delete subscription -n $EXISTING_NS \
    openshift-cert-manager-operator --ignore-not-found
  CSV=$(oc --context=$cluster get csv -n $EXISTING_NS -o name 2>/dev/null | grep cert-manager)
  [ -n "$CSV" ] && oc --context=$cluster delete $CSV -n $EXISTING_NS
done
```
